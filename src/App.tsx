...
const PortfolioContent = () => {
  ...
  const categories = ['ALL', 'UI/UX', 'GRAPHIC', 'MOTION', 'PHYSICAL'];
  ...
  const projects = [
    {
      ...
      desc: 'An exploration of temporal interfaces using non-linear navigation patterns. Time is perceived not as a strict sequence but a multi-dimensional array of experiences.'
    },
    {
      id: 2,
      title: 'Neo-Brutalism Manifesto',
      category: 'GRAPHIC',
      year: '2025',
      cover: 'https://images.unsplash.com/photo-1574169208507-84376144848b?q=80&w=2000&auto=format&fit=crop',
      images: [
        'https://images.unsplash.com/photo-1627398240316-ea3900115de2?q=80&w=2000&auto=format&fit=crop'
      ],
      desc: 'A typographical study on brutalist forms. Rejecting corporate homogeneity in favor of raw, unpolished, and intensely legible layouts.'
    },
    {
      id: 3,
      ...
      category: 'MOTION',
      year: '2025',
      cover: 'https://images.unsplash.com/photo-1550751827-4bd374c3f58b?q=80&w=2000&auto=format&fit=crop',
      images: [],
      desc: 'Generative motion graphics driven by real-time audio reactive data. Particle systems mimicking cellular automata and organic fluid flow.'
    },
    {
      id: 4,
      title: 'Echo Chambers',
      category: 'PHYSICAL',
      year: '2024',
      cover: 'https://images.unsplash.com/photo-1604871000636-074fa5117945?q=80&w=2000&auto=format&fit=crop',
      images: [
        'https://images.unsplash.com/photo-1550684376-efcbd6e3f031?q=80&w=2000&auto=format&fit=crop'
      ],
      desc: 'An interactive spatial installation constructed with responsive mesh networks and localized soundscapes designed for sensory deprivation.'
    },
    {
      id: 5,
      ...
      category: 'UI/UX',
      year: '2026',
      cover: 'https://images.unsplash.com/photo-1605806616949-1e87b487cb2a?q=80&w=2000&auto=format&fit=crop',
      images: [],
      desc: 'A minimalist spatial computing OS concept stripped of all skeuomorphism, relying purely on spatial hierarchy and gesture-based invisible controls.'
    },
    {
      id: 6,
      ...
      category: 'GRAPHIC',
      year: '2025',
      cover: 'https://images.unsplash.com/photo-1541701494587-cb58502866ab?q=80&w=2000&auto=format&fit=crop',
      images: [],
      desc: 'Print series exploring color perception, optical illusions, and the physiological effects of highly saturated contrasting hues on the retina.'
    }
  ];
  ...
  return (
    <div className="h-full bg-[#111] text-[#eee] flex flex-col overflow-hidden font-sans rounded-lg">
      {/* Navbar / Header */}
      <div className="flex flex-col md:flex-row md:items-end justify-between px-8 py-10 shrink-0 gap-6">
        <div>
          <h1 className="text-4xl md:text-5xl font-bold tracking-tighter uppercase leading-none">Creative</h1>
          <h1 className="text-4xl md:text-5xl font-bold tracking-tighter text-[#666] uppercase leading-none">Archive©</h1>
          <p className="mt-3 text-xs md:text-sm font-mono tracking-widest text-[#888] uppercase">Selected Works 2024—2026</p>
        </div>
        
        <div className="flex flex-wrap gap-4 md:gap-6 uppercase text-xs md:text-sm font-bold tracking-wider overflow-x-auto pb-2 custom-scrollbar">
          {categories.map(cat => (
            <button 
              key={cat}
              onClick={() => setActiveTab(cat)}
              className={`transition-colors pb-1 border-b-2 whitespace-nowrap ${activeTab === cat ? 'border-white text-white' : 'border-transparent text-[#666] hover:text-[#aaa]'}`}
            >
              {cat}
            </button>
          ))}
            ...
            {filteredProjects.map((project, idx) => (
              ...
              >
                <div className="relative aspect-[4/5] overflow-hidden bg-[#222]">
                  ...
                  {/* Hover Overlay Info lg screens */}
                  <div className="absolute inset-0 p-6 hidden md:flex flex-col justify-between opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                    <div className="flex justify-between items-start w-full">
                      <span className="px-3 py-1 bg-white text-black text-xs font-bold uppercase tracking-widest rounded-full">{project.category}</span>
                      <span className="text-white text-xs font-mono bg-black/50 px-2 py-1 backdrop-blur-md rounded">{project.year}</span>
                    </div>
                    <div className="bg-black/40 backdrop-blur-md p-4 border border-white/10 translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                      <p className="text-sm text-gray-200 line-clamp-2">{project.desc}</p>
                    </div>
                  </div>

                  {/* Always showing tags on mobile */}
                  <div className="absolute top-4 left-4 right-4 flex justify-between items-start md:hidden">
                      <span className="px-2 py-1 bg-white text-black text-[10px] font-bold uppercase tracking-widest rounded-full shadow-lg">{project.category}</span>
                      <span className="text-white text-[10px] font-mono bg-black/70 px-2 py-1 backdrop-blur-md rounded shadow-lg">{project.year}</span>
                  </div>
                </div>
                <div className="mt-4 flex items-baseline justify-between gap-4">
                  <h3 className="text-lg md:text-xl font-bold tracking-tight uppercase truncate">{project.title}</h3>
                  <span className="text-xs font-mono text-[#666]">0{project.id}</span>
                </div>
              </motion.div>
            ))}
          </AnimatePresence>
      ...
      <AnimatePresence>
        {selectedProject && (
          <motion.div 
            ...
            className="absolute inset-0 z-50 bg-[#0a0a0a] overflow-y-auto custom-scrollbar"
          >
            <div className="min-h-full w-full flex flex-col">
              {/* Modal Header */}
              <div className="sticky top-0 z-20 flex justify-between items-center p-6 md:p-8 mix-blend-difference text-white pointer-events-none">
                <span className="font-mono text-xs md:text-sm uppercase tracking-widest">{selectedProject.year} // {selectedProject.category}</span>
                <button 
                  onClick={() => setSelectedProject(null)}
                  className="w-10 h-10 md:w-12 md:h-12 flex items-center justify-center rounded-full bg-white/20 hover:bg-white text-white hover:text-black transition-colors pointer-events-auto backdrop-blur-md"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              {/* Modal Content */}
              <div className="px-6 md:px-8 pb-24 max-w-7xl mx-auto w-full -mt-16 md:-mt-20">
                <motion.div 
                  initial={{ y: 30, opacity: 0 }}
                  animate={{ y: 0, opacity: 1 }}
                  transition={{ delay: 0.2, duration: 0.6 }}
                >
                  <h1 className="text-[12vw] md:text-[8vw] leading-[0.9] font-bold tracking-tighter uppercase mb-12 hyphens-auto mt-24">{selectedProject.title}</h1>
                </motion.div>
                
                <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-12 mb-16 md:mb-20">
                  <div className="md:col-span-4 lg:col-span-3 font-mono text-xs md:text-sm text-[#888] flex flex-row md:flex-col gap-8 md:gap-8 overflow-x-auto pb-4 md:pb-0">
                    <div className="min-w-[120px]">
                      <h4 className="text-white mb-2 md:mb-3 uppercase tracking-widest font-sans font-bold">Role</h4>
                      <p className="leading-relaxed">Creative Direction<br/>Visual Design<br/>Interaction Design</p>
                    </div>
                    <div className="min-w-[200px]">
                      <h4 className="text-white mb-2 md:mb-3 uppercase tracking-widest font-sans font-bold">Context</h4>
                      <p className="leading-relaxed">A conceptual exploration challenging traditional paradigms of user interfaces in the context of advanced computing.</p>
                    </div>
                  </div>
                  <div className="md:col-span-8 lg:col-span-9 text-lg md:text-2xl lg:text-3xl leading-relaxed text-[#ccc] font-medium tracking-tight">
                    {selectedProject.desc} Everything we knew about structured communication and interfaces is changing. The viewport is no longer a rigid boundary, but a window into a limitless, experiential canvas.
                  </div>
                </div>
                ...
                <div className="mt-24 pt-12 border-t border-white/10 flex justify-between items-center text-white/50 hover:text-white transition-colors cursor-pointer" onClick={() => setSelectedProject(null)}>
                  <span className="font-mono text-sm uppercase tracking-widest">Back to Archive</span>
                  <X className="w-6 h-6" />
                </div>
              </div>
            </div>
